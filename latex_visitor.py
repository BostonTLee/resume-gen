...
class LatexVisitor:
    def __init__(self, filename):
        ...
        self.document = Document(geometry_options={"margin": "1in", "top": "0.5in"}, page_numbers=False)
    ...
    def visitResume(self, resume: Resume):
        ...
        self.document.packages.append(Package("hyperref", options="colorlinks, urlcolor=blue"))
        ...
        #                                 ))
        # FIXME
        self.document.append(utils.NoEscape(r"\titleformat{\section}{\Large\scshape}{\thesection}{1em}{}[{\titlerule[0.8pt]}]"))
    ...
    def visitJob(self, job: Job):
        ...
        with self.document.create(MiniPage(arguments=utils.NoEscape(r"0.75\textwidth"))):
            with self.document.create(FlushLeft()):
                self.document.append(utils.bold(job.title))
                self.document.append(LineBreak())
                self.document.append(utils.bold(job.employer))
        with self.document.create(MiniPage(arguments=utils.NoEscape(r"0.25\textwidth"))):
            with self.document.create(FlushRight()):
                self.document.append(Command("raggedleft"))
                self.document.append(utils.bold(utils.NoEscape(format_date_range(job.start_date, job.end_date))))
                self.document.append(LineBreak())
    ...
    def visitEducation(self, education: Education):
        ...
        with self.document.create(MiniPage(arguments=utils.NoEscape(r"0.25\textwidth"))):
            with self.document.create(FlushRight()):
                self.document.append(Command("raggedleft"))
                self.document.append(utils.NoEscape(format_date_range(education.start_date, education.end_date)))
                self.document.append(LineBreak())
